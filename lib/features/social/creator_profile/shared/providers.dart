...
final creatorProfileNotifierProvider =
...
final resolvedCreatorProfileProvider = Provider.family<CreatorProfile?, String>(
  (ref, accountId) {
...
class CreatorProfileEditNotifier extends StateNotifier<CreatorProfileEditData> {
...
final creatorSpecializationIdsProvider = FutureProvider.family
    .autoDispose<List<String>, String>(
      (ref, accountId) async {
...
final creatorSpecializationsWithPrimaryProvider = FutureProvider.family
    .autoDispose<Map<String, bool>, String>(
      (ref, accountId) async {
        if (accountId.isEmpty) return const {};
        final either = await ref
            .read(creatorProfileRepositoryProvider)
...
final updateShowcaseNotifierProvider =
    StateNotifierProvider<UpdateShowcaseNotifier, UpdateShowcaseState>(
      (ref) =>
          UpdateShowcaseNotifier(ref.watch(creatorProfileRepositoryProvider)),
    );
...
final showcasedBadgesProvider = Provider<List<BadgeAward>>(
  (ref) => ref
      ...
      .maybeWhen(
        loadSuccess: (badges) => badges.where((b) => b.isShowcased).toList()
          ..sort(
            (a, b) =>
...
final creatorConnectedAccountsProvider = FutureProvider.family
    .autoDispose<List<SocialAccountSummary>, String>(
      (ref, accountId) async {
          ...
          for (final item in raw.whereType<Map>()) {
            ...
            summaries.add(
              SocialAccountSummary(
                ...
                followerCount:
                    int.tryParse((item['followerCount'] ?? '0').toString()) ??
                    0,
              ),
            );
          }
...
final creatorPlatformAvatarUrlsProvider = Provider.family
    .autoDispose<List<String>, String>((ref, accountId) {
      final accounts = ref
...
/// accountId is ignored (endpoint is user-scoped via auth).
final creatorConnectedSocialIdsProvider = Provider.family
    .autoDispose<Set<String>, String>(
      (ref, accountId) {
        final asyncAccounts = ref.watch(
          creatorConnectedAccountsProvider(accountId),
        );
...
final youtubeChannelProvider = FutureProvider.family
    .autoDispose<YouTubeChannel?, String>(
      (ref, channelId) async {
...
final creatorNicheNamesProvider = FutureProvider.family
    .autoDispose<List<String>, String>(
      (ref, accountId) async {
        if (accountId.isEmpty) return const [];
