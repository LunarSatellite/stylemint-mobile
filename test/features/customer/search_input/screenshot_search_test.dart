...
void main() {
  group('ScreenshotSearch', () {
    ...
    test('one image leaves the device, once, as a data URI', () async {
      ...
      expect(source.requests.single.single, startsWith('data:image/png;base64,'));
    });
    ...
    test('an unexpected throw is still unavailable rather than a crash',
        () async {
      final outcome = await ScreenshotSearch(
  ...
  group('probeVisualSearch', () {
    test('available:true is ready, available:false hides the entry point',
        () async {
      expect(
